    ...
    if request.method == "POST":
        ...
        if not user_created:
            current_app.logger.error(
                "Signup failed - database create error for account=%s", email_fp
            )
        ...
        if email_sent:
            ...
            flash(
                ...
                "email. Please use resend verification from login.",
                "warning",
            )
...
@limiter.limit(
    ...
    deduct_when=_deduct_on_failed_auth,
)
def login():
    if request.method == "POST":
        email = normalize_email(request.form.get("email", ""))
        ...
        if not user_data.get("email_verified", True):
            ...
            flash(
                "Please verify your email before logging in. You can resend "
                "the verification email below.",
                "warning",
            )
...
def profile():
    ...
    if request.method == "POST":
        ...
        if action == "update_profile":
            ...
            if not updated_user:
                flash(
                    "We could not update your profile because your account was not found. "
                    "Please log in again.",
                    "danger",
                )
                logout_user()
            ...
            if not stored_password_hash or not check_password_hash(
                stored_password_hash, current_password
            ):
            ...
            if not updated_user:
                flash(
                    "We could not change your password because your account was not found. "
                    "Please log in again.",
                    "danger",
                )
                logout_user()
...
def resend_verification():
    ...
    if send_verification_email(email):
        ...
        flash(
            "We could not send a verification email right now. Please try again shortly.", "danger"
        )
    return redirect(url_for("auth.login"))
